...
export default function CardStocklist({ data }) {
  return (
    <>
                ...
                _.map(data, (item) => (
                  <tr key={item.id}>
                    <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                      {_.get(item, 'attributes.stocksTicker')}
                    </th>
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {_.get(item, 'attributes.price')}
                    </td>
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {_.get(item, 'attributes.quantity')}
                    </td>
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {_.get(item, 'attributes.industry.data.attributes.name')}
                    </td>
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-right">
                      <i className="fas fa-arrow-up text-emerald-500 mr-4"></i>
                      {_.floor(item.attributes.price * item.attributes.quantity, 2)}$
                    </td>
                  </tr>
                ))
              }
